...
try {
    let input = fs.readFileSync('workInput.txt', 'utf8')
...
async function processRangeAsync(start, end) {
    ...
    for (let j = start; j < end; j++) {
        let location = await getLocation(j);
...
async function findMinLocation() {
    let promises = [];

    for (let i = 0; i < seedArray.length; i += 2) {
        promises.push(processRangeAsync(seedArray[i], seedArray[i] + seedArray[i + 1] - 1));
    }

    const locations = await Promise.all(promises);
    return Math.min(...locations.filter(location => location !== undefined));
}

// Call the asynchronous function and handle the result
findMinLocation().then(minLocation => {
